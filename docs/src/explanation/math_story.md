...
to *run* the pipeline see [The five-verb pipeline](@ref), and to look up a symbol
see the [Notation glossary](@ref).
...
```math
\textbf{$\eta$-idempotent} \iff \|\Phi^2 - \Phi\|_{\mathrm{cb}} \le \eta
\qquad (\texttt{tex:354}).
```
...
``\rho`` alone — see [Certified arithmetic](@ref).
...
the package's strongest cross-check — the [The η = 0 oracle](@ref).
...
!!! warning "The product is ``\tilde\Phi(XY)``, not ``XY``"
    The multiplication on ``A`` is the Choi–Effros star ``X \star Y =
...
!!! note "The axioms hold only up to ``\varepsilon``"
    Associativity, submultiplicativity, the ``C^*`` identity, and the unit laws
...
test that cannot fail. This is the subject of [Dimension-independence](@ref), and
...
the same bound (see [The constructive mandate](@ref)):
...
!!! warning "``factorize`` has a tighter domain than the rest of the pipeline"
    ``\Upsilon`` is built by a power-series functional calculus whose convergence
    ...
    domain](@ref) and [Design decisions and known limits](@ref).
...
- Run it: [The five-verb pipeline](@ref) and [The η = 0 oracle](@ref).
- Trust it: [Certified arithmetic](@ref) (the cross-check ladder).
- The central claim: [Dimension-independence](@ref).
- Look up a symbol: the [Notation glossary](@ref).
- The constructivisation in detail: [The constructive mandate](@ref).
