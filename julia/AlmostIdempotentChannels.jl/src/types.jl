...
"""
    ChannelFactorization(source, B, Δ, Υ, delups, upsdel, eta_proxy, eps_used)

The approximate channel factorization Φ ≈ Δ Υ through a genuine C* algebra B
(th_factorization, approximate_algebras.tex:2730-2899). Δ: B → B(H) (the UCP
encode OBSERVABLE map) and Υ: B(H) → B (the UCP decode observable map) satisfy
the by-construction targets ΔΥ ≈ Φ̃, ΥΔ ≈ 1_B, certified by two cb-norm
round-trip brackets `delups = ‖ΔΥ − Φ‖_cb` (.tex:2733) and
`upsdel = ‖ΥΔ − 1_B‖_cb` (.tex:2739). `eta_proxy`/`eps_used` are the shim
sentinels (≈ 0 at η = 0). The state-space CHANNELS are the duals Dec = Δ*,
Enc = Υ* (Appendix B1); `encode`/`decode` (bead J4) expose those.
...
Fields store Δ/Υ as Kraus lists; J4 populates this from the C pipeline.
"""
struct ChannelFactorization
    source::UCPMap
    B::CStarAlgebra
    Δ::Vector{Matrix{ComplexF64}}   # encode observable map Δ: B → B(H)
    Υ::Vector{Matrix{ComplexF64}}   # decode observable map Υ: B(H) → B
