...
### C20. The prop_Gamma `gamma_j` is UNIQUELY extractable from the stored `d.Gamma` by a DATA-DRIVEN solve (the match-`d.Gamma` tooth pins it); the result is maximally-mixed `gamma_j = (1/dim E_j) 1` ONLY for the *uniform* noiseless-subsystem oracle — the extraction is GENERAL, a non-uniform `d.Gamma` yields a non-uniform `gamma_j` (proven by the `make_ns_weighted` oracle)
- **Where surfaced.** bead aic-ynu I3 (`src/aic_idemp_gamma_kraus.c`, `tests/test_gamma_kraus.c`). prop_Gamma (`.tex:2106-2122`) says ANY UCP `Gamma : B(M) -> A` with `Gamma w = 1_A` has the form `Gamma_j(X) = Tr_{E_j}(W_j X W_j^dag (1_{L_j} (x) gamma_j))` for density matrices `gamma_j` on `E_j`. The paper does NOT fix WHICH `gamma_j` the stored `d.Gamma = Delta^dag Lambda` (`aic_idemp_build.c`, `.tex:2065`) uses.
- **Finding.** For the canonical nontrivial-multiplicity oracle `make_noiseless_subsystem(2,2)` (`Phi(X)=(Tr_E X)(x)(1_E/dE)`, `E_1=C^2`), solving the linear system `Tr_{E_1}(W_1 X W_1^dag (1(x)gamma_1)) = Gamma_1(X)` over a basis of `B(M)` gives `gamma_1 = (1/2) 1_{E_1}` — the **maximally-mixed** density matrix (Hermitian, PSD, trace 1; match residual `3.6e-16`). So `d.Gamma` is the maximally-mixed (uniform) conditional expectation **for this oracle**. This is NOT presumed — it is DETERMINED by the solve, and the extraction is GENERAL: the `make_ns_weighted` oracle (a weighted noiseless subsystem, `sigma=diag(0.8,0.2)`) is an exactly-idempotent NON-uniform `d.Gamma` and the solve correctly returns the non-uniform `gamma_0 = diag(0.2,0.8)` (match residual `~1e-16`). A hardcoded `gamma=1/dE` would pass the uniform oracles but fail `make_ns_weighted` — that is the load-bearing distinction.
- **The tooth.** `gamma_j |-> Gamma_j` is INJECTIVE (`W_j` an isometry onto block `j`, so `W_j X W_j^dag` sweeps all of `B(block)`), so the `gamma_j` is UNIQUE and the **match-`d.Gamma`** invariant — `||w(Gamma_kraus(E_ab)) - w(d.Gamma(E_ab))||_op <= 1e-10` on a basis of `B(M)`, embedded via the *-monomorphism `w` (gauge-safe) — pins the `gamma_j` extraction AND the `Tr_{E_j}` convention AND the `W_j` usage at once. Mutation-proven: a NON-uniform `gamma_1 = diag(0.8,0.2)` (a valid density matrix, the WRONG conditional expectation) blows the residual to `0.2965` (the uniform-replacement `1/dE` does NOT bite here precisely BECAUSE `d.Gamma` is already uniform — the perturbation must be non-uniform). A swapped `Tr_{E_j} -> Tr_{L_j}` makes the eq-Gamma and Choi-`L_j` forms DISAGREE and the production self-cert aborts.
- **Subtlety (numerical).** The maximally-mixed `gamma_j = (1/dim E_j)1` has a DEGENERATE spectrum, so the purification `xi_j` (for the Kraus `L_j`) needs the degeneracy-robust `aic_mat_eig_hermitian_subspaces` (`sqrt(gamma) = sum_c sqrt(lambda_c) Pi_c`), NOT the simple-spectrum `aic_mat_eig_hermitian` (which aborts on the repeat). The raw double-path least-squares `gamma_j` is symmetrized (`(gamma+gamma^dag)/2`) AFTER a lenient Hermiticity check, so the tight-Hermiticity eig routine accepts it.

### C21. The `docs/adversarial/nla.md:621-628` Kahan "Math" formula `K[i][i]=s^i` is the WRONG convention — it does NOT exhibit the catastrophic cancellation; the genuinely-pathological matrix is the MATLAB `gallery('kahan')` form `K[i][i]=c^i, K[i][j]=-s c^i`
- **Where surfaced.** bead aic-dbo.2, nla-5b generator (`tests/aic_adversarial_kahan.c::aic_adv_kahan`, `tests/test_adversarial.c::test_gen_kahan`). The catalogue's 5b entry is internally inconsistent: its "Math" block writes `K[i][i] = s^i`, `K[i][j] = -c s^{i+j-1}` (sin-graded), but its PROSE knob ("theta near pi/2, s~0 is lethal") and the classic condition number `kappa ~ (2c/s^{n-1})^n` both describe the cos-graded MATLAB `gallery('kahan')` / Higham form instead. The two cannot both be right; `s=sin(theta)` makes "small theta => s~1" (the prose) FALSE for the sin-graded formula.
- **The sin-graded formula is toothless (MEASURED, prec=256).** Building `K[i][i]=s^i`, `K[i][j]=-c s^{i+j-1}`: at the lethal small-theta end (`theta=0.3`, n=8) `sigma_min = 1.97e-4 = s^7 = smallest diagonal` to 4 figures (`sigma_min/mindiag = 1.0000`). There is NO separation between the smallest singular value and the smallest diagonal entry — exactly the cancellation that DEFINES the Kahan pathology is ABSENT. So the literal nla.md formula does not stress the QR-rank-revealer at all.
- **The gallery('kahan') form has the genuine pathology (MEASURED).** `c=cos(theta)`, `s=sin(theta)`, `K[i][i]=c^i`, `K[i][j]=-s c^i` (j>i). At `theta=1.4` (c=0.17), n=8: `sigma_min = 5.84e-8`, `kappa = 4.8e7`, smallest diagonal `c^7 = 4.10e-6`, so `sigma_min/mindiag = 0.014` — the true smallest singular value is ~70x BELOW the smallest pivot a column-pivoting QR sees. Knob direction: SMALL theta (c->1) MILD, LARGE theta (c->0, toward pi/2) LETHAL — matching nla.md's PROSE.
- **Resolution (Law 3).** Constructivize the gallery form (the matrix that actually breaks the routine), not the catalog typo. The generator ships the cos-graded form; the self-test pins `sigma_min` collapsing ~4.4e4x mild->lethal, `kappa` exploding 3.56 -> 3.05e5, and `sigma_min << c^{n-1}` at the lethal end (ratio 0.056 at n=6, theta=1.4). The certified arb `sigma_min` (`aic_mat_singular_values`) agrees with the double LAPACK SVD at n=6 (rung-2 cross-check); both resolve it. NOTE the arb simple-Gram SVD FAIL-LOUD-ABORTS (`aic_mat_int_eig_certified: clustered eigenvalues? raise prec`) once the singular values cluster — at `theta=1.45`, n>=16 — which is correct Rule-4 behaviour, so the self-test stays at n=6 where they are distinct.

### C22. The Julia-ABI shims C2–C5 (`aic-exa.3`): the RECTANGULAR eig-free cb bracket, the SOLVER-FREE `‖v‖_cb` route, and the `decode∘encode` round-trip target is `P_B` not `1_B`
- **Status:** CONFIRMED + RESOLVED (bead aic-exa.3 [C]; `src/aic_assoc_shim.c` (C2), `src/aic_iso_shim.c` (C3), `src/aic_factorize_artifacts_shim.c` + `_artifacts2_shim.c` (C4/C5), `src/aic_cbnorm.c` (the new rect bracket core); `tests/test_shim_assoc_iso.c`, `tests/test_shim_factorize_artifacts.c`). The flat-double `_d` ABI extension for the Julia package (design §4.3, Appendix B).
- **(a) C3 `‖v‖_cb` / `‖v⁻¹‖_cb` SOLVER-FREE bracket needs a NEW rectangular core — the self-map `aic_cbnorm_eigfree_ball_choi(J,n)` is `n²×n²`-ONLY (`.tex:347-352`, §C12).** `v: B→A` is a RECTANGULAR map; its Choi `J(v*)` (the §C12.O2 GOLDEN-RULE adjoint Choi, built DIRECTLY via `aic_opspace_choi_vstar`, NOT by transpose) is `(N·n_B)×(N·n_B)`, which is `n²` only when `N·n_B` is a perfect square — so the self-map routine's `assert(side==n²)` would fire, and its tighter `hi=2‖J‖_F` upper relies on the self-map `P+Q=I_{n²}` primal that does NOT hold for a rectangular map (abusing it would be UNRIGOROUS). The constructive route (Law 3, faithful generalization of the self-map derivation): `aic_cbnorm_eigfree_ball_choi_rect(lo,hi,J,d_in,d_out)` with the RIGOROUS bracket `‖f‖_⋄ ∈ [‖J‖_F/d_in, d_out·‖J‖_F]` (lower = maximally-entangled INPUT state on `M_in`, `‖J‖_1/d_in`; upper = the dual feasible point `Y0=Y1=‖J‖_F·I`, giving `λ_max(Tr_out Y0)=‖J‖_F·d_out` — both eig-free, solver-free, never abort). Derivation in `src/aic_cbnorm.c`. CROSS-CHECKED (ladder rung 4): on `mixconj(6,2,0.03)` the eig-free bracket CONTAINS the INDEPENDENT MOSEK-pinned SDP values `‖v‖_cb=1.0019683734 ∈ [0.4502,5.4024]`, `‖v⁻¹‖_cb=1.5353598357 ∈ [0.9783,11.7392]` (§C12.O2). At the η=0 complete-isometry oracle the cb LOWER endpoint is EXACTLY 1 (identity, dim-independent) / 0.7071 (block_cond_exp, dim-independent across n=4,6,8) — the §D2 dimension-independence canary lives on the LOWER endpoint (the `hi` grows `O(n²)` by the loose-bracket design, expected).
- **(b) The `decode∘encode` round-trip target is the CONDITIONAL EXPECTATION `P_B`, NOT the full identity `1_{M_{n_B}}` (the §C14(b) trap re-bites the C5 dual channels).** The C5 state round-trip `Dec∘Enc = (Υ Δ)*` and `Υ Δ = P_B` (block-diagonal conditional expectation), NOT `1_{M_{n_B}}`: `Δ` reads only B's block-diagonal coordinates, so `(Dec∘Enc)(E_pq)=0` for OFF-block `E_pq` while `1_B(E_pq)=E_pq` there. A naive round-trip oracle comparing to the FULL identity reads `1.0` on every off-block unit even at η=0 (MEASURED on `block_cond_exp(4,2)`: worst-ALL-units `=1.000`, worst-IN-BLOCK `=0.000`) — the exact §C14(b) "test that cannot pass". The fix (in `test_shim_factorize_artifacts.c::roundtrip`): sweep only IN-BLOCK units (the regime where `Υ Δ = id_B`). The single-block oblique fixtures (`make_mixconj`, `m=1`) have `P_B=1` so all units are in-block — that is why the η>0 mutation tooth (b) is exercised there.
- **(c) The Appendix-B1 dec/enc binding MUTATION tooth is SHARP only on an OBLIQUE η>0 fixture with `N≠n_B` (`make_mixconj`).** The η=0 identity oracle has `N=n_B` and `Dec=Enc=identity`-shaped, so swapping `dec_kraus`↔`enc_kraus` is STRUCTURALLY INVISIBLE (round-trip unchanged at `8.6e-78` either way) — the §C13 dual-direction trap is blind to it. `make_mixconj(d,m)` gives `N=d ≠ n_B=m` and genuinely-different Dec/Enc maps: the CORRECT `decode∘encode` round-trip is `O(η)` (`1.5e-2` at η`=1.3e-2`), the SWAPPED one is `O(1)` (`1.0–2.5`), a **72–163× separation** (mutation-proven RED in `tests/test_shim_factorize_artifacts.c`; a shim-level cross of `dec_kraus`↔`enc_kraus` drives the CORRECT-binding round-trip to `1.04`, RED, then reverted). This is the test that proves B1.
- **(d) Dimension-independence canary on the C3 `iso_def/η`: `mixconj(4,2)` is the known geometry-FRAGILE outlier (§D2/§C11).** Measured `iso_def/η`: (4,2)→`12.05`, (5,3)→`0.43`, (6,3)→`0.46`. The `(4,2)` `dim_A=2` degenerate-wrapper (§C16) is the same hard-geometry outlier §D2 documents (and `cstar_build` T3 uses the GENEROUS `c<20` per-instance bound for). So the shim's dim-GROWTH canary runs on the well-behaved pure-dim sweep `(5,3)→(6,3)` (same `M_3` block, only ambient `n` grows) where `c` stays flat (`0.43→0.46`, no upward trend), with `c<20` per-instance — matching T3. The round-trip-defect `/η` (a DIFFERENT, well-behaved quantity) is dimension-flat across all three: `1.15, 1.16, 1.15`.
