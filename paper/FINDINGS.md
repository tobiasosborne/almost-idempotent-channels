...
### C20. The stored `d.Gamma` (= `Delta^dag Lambda`, the th_idemp_structure conditional expectation) is the MAXIMALLY-MIXED prop_Gamma form — `gamma_j = (1/dim E_j) 1_{E_j}` — and the prop_Gamma `gamma_j` is UNIQUELY extractable to match it (the match-`d.Gamma` tooth pins it)
